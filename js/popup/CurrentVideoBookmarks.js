...
export const setCapturedFramesAndRender = () => {
	chrome.runtime.sendMessage(
		...
		async (currentVideoBookmarksWithFrames) => {
			...
			state.currentVideoBookmarksWithDataUrlByTime = arrayToObjectByKey(
...
export const renderBookmarkElementsForCurrentVideo = async () => {
	...
	const bookmarksWithProgress =
		state.currentVideoBookmarks &&
		state.currentVideoBookmarks.map((bookmark) => {
			return {
				...bookmark,
				finished: false,
			};
 ...
 */
export const addNewBookmarkElem = async (
	bookmarkListElem,
	bookmark,
	dataUrl,
	isLastIndex,
	index,
	bookmarksWithProgress
) => {
